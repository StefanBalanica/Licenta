            ...
            <form [formGroup]="pwForm" (ngSubmit)="changePassword()" class="form">
              ...
              <div class="field">
                ...
                <ul class="pw-rules" *ngIf="newPwValue.length > 0">
                  <li [class.ok]="!newPwErrors['minLength']"><span class="ri">{{ !newPwErrors['minLength'] ? '✓' : '○' }}</span> Minimum 8 caractere</li>
                  <li [class.ok]="!newPwErrors['uppercase']"><span class="ri">{{ !newPwErrors['uppercase'] ? '✓' : '○' }}</span> Literă mare (A-Z)</li>
                  <li [class.ok]="!newPwErrors['lowercase']"><span class="ri">{{ !newPwErrors['lowercase'] ? '✓' : '○' }}</span> Literă mică (a-z)</li>
                  <li [class.ok]="!newPwErrors['digit']"><span class="ri">{{ !newPwErrors['digit'] ? '✓' : '○' }}</span> Cifră (0-9)</li>
                  <li [class.ok]="!newPwErrors['special']"><span class="ri">{{ !newPwErrors['special'] ? '✓' : '○' }}</span> Caracter special (!&#64;#$%)</li>
                </ul>
              </div>
              ...
              <button type="submit" class="btn-primary" [disabled]="pwForm.invalid || pwLoading">
                ...
                <span *ngIf="pwLoading" class="spin-wrap"><span class="spin"></span>Se salvează…</span>
              </button>
            </form>
          </section>
          ...
          <section class="card card-danger">
            <div class="section-head">
              ...
              <div>
                ...
                <h2 class="section-title">Șterge contul</h2>
              </div>
            </div>
            <p class="danger-desc">Ștergerea contului este <strong>ireversibilă</strong>. Toate jocurile, personajele și dispozitivele asociate vor fi șterse permanent.</p>
            ...
            <div *ngIf="confirmDelete" class="confirm-box">
              ...
              <div class="confirm-acts">
                ...
                <button class="btn-danger" [disabled]="deleteConfirmText !== 'STERGE' || deleteLoading" (click)="deleteAccount()">
                  <span *ngIf="!deleteLoading">Șterge definitiv</span>
                  <span *ngIf="deleteLoading" class="spin-wrap"><span class="spin spin-red"></span>Se șterge…</span>
                </button>
              </div>
            </div>
...
export class ProfileComponent implements OnInit {
  ...
  async deleteAccount() {
    ...
    try {
      ...
      alert(err.error?.message || 'Ștergerea a eșuat. Încearcă din nou.');
